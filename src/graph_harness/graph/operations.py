...
_DEFAULT_OPERATIONS = [
    ...
    GraphOperation("list_service_principals", "GET", "/servicePrincipals", "List service principals.", optional_params=("$select", "$filter", "$top")),
    GraphOperation("list_oauth_permission_grants", "GET", "/oauth2PermissionGrants", "List OAuth permission grants.", optional_params=("$filter", "$top")),
    GraphOperation("delete_oauth_permission_grant", "DELETE", "/oauth2PermissionGrants/{grant_id}", "Delete an OAuth permission grant.", read_only=False),
]
